...
# 读取CSV文件
file_path = '../data/High-resolution data/tran5_veg_30m.csv'  # 替换为您的CSV文件路径
...
step_size = 80     # 每次移动84列
reversed_transposed_data = reversed_transposed_data.T

# 计算滑动窗口的平均值（忽略NaN）
means = []

# 计算可以应用窗口的起始位置
for start_col in range(0, data.shape[0] - window_size + 1, step_size):
    window = reversed_transposed_data.iloc[:, start_col:start_col + window_size]  # 提取当前窗口的所有行和指定列
    # 计算每个窗口的平均值，忽略NaN
    window_mean = window.mean().mean()  # 对窗口内的每列计算平均值，然后再计算这些平均值的平均数
    means.append(window_mean)  # 添加到列表

# 将结果转换为DataFrame
mean_df = pd.DataFrame(means)  # 创建DataFrame并指定列名

# 将滑动窗口的平均值保存到CSV文件
mean_df.to_csv('./data/rolling_average_values.csv', index=False)  # 保存为CSV文件，不保存索引
